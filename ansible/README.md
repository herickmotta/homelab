...
- `herickmotta.homelab.host_metrics`: pinned `node_exporter` as a systemd
  ...
  Optional textfile probes publish loopback TCP connects (no payload;
  `address` label), allowlisted Compose liveness, and a full Docker
  inventory (health, restarts, IPs, published ports, network gateways)
  without exposing the Docker socket to other roles.
...
- `herickmotta.homelab.hermes_agent`: pinned official Nous Research Hermes
  ...
  and reaches the data guest Kong API on TCP 8000. Dump age identity and the
...
- `herickmotta.homelab.data_platform`: pinned minimal self-hosted Supabase
  ...
  age-encrypted dumps land on a site backup directory (often VirtioFS from
  `iron`). S3 upload is optional and off by default. Disabling the role
